...
void gyro_signals(void) {
  ...
  float AccXLSB = TwosComp(Wire.read() << 8 | Wire.read());
  float AccYLSB = TwosComp(Wire.read() << 8 | Wire.read());
  float AccZLSB = TwosComp(Wire.read() << 8 | Wire.read());
  Wire.beginTransmission(0x68);
  Wire.write(0x1B); 
  Wire.write(0x8);
  Wire.endTransmission();     
  ...
  Wire.endTransmission();
  Wire.requestFrom(0x68,6);
  float GyroX=TwosComp(Wire.read()<<8 | Wire.read());
  float GyroY=TwosComp(Wire.read()<<8 | Wire.read());
  float GyroZ=TwosComp(Wire.read()<<8 | Wire.read());
...
void loop() {
  ...
  Serial.print(KalmanAngleRoll +.32);
  Serial.print(" Pitch Angle [°] ");
  Serial.println(KalmanAnglePitch +4.2);
