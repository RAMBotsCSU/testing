...
void look_up_or_down(float left_trigger_val, float right_trigger_val){
  ...
  front_legs_z = ema(left_legs_z, map(left_trigger, 0, 1, upperBound, lowerBound), alpha);
  back_legs_z = ema(left_legs_z, map(left_trigger, 0, 1, upperBound, lowerBound), alpha);


      kinematics (1, 0, 0, front_legs_z, 0, 0, 0, 0, 0);   // front right
      kinematics (2, 0, 0, back_legs_z, 0, 0, 0, 0, 0);   // front left
      kinematics (3, 0, 0, legs_z, 0, 0, 0, 0, 0);   // back left
      kinematics (4, 0, 0, legs_z, 0, 0, 0, 0, 0);   // back right  
}

float angle(float gyro_val){
  float angle = gyro_val;
  if(angle > 45){
    angle = 45;
    }
  if(angle < -45){
