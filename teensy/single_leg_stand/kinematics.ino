...
void kinematics (int leg, float xIn, float yIn, float zIn, float roll, float pitch, float yawIn, int interOn, int dur) {
      ...
      if (interOn == 1) {     
            ...
            if (leg == 1) {        // front right
                z = interpZ.go(zIn,dur);
                x = interpX.go(xIn,dur);
                y = interpY.go(yIn,dur);
                yaw = interpT.go(yawIn,dur);
            }
      ...
      if (leg == 1) {     // front right
          ...
          Serial.print("shoulderAngleCounts: ");   
          Serial.println(shoulderAngleCounts);
          // Serial.print("Interpolation Active");   
          // Serial.println(kneeAngleCounts);
          if (!isnan(shoulderAngleCounts) && !isnan(kneeAngleCounts)) {
            driveJoints (10, shoulderAngleCounts);    // front right shoulder
            driveJoints (11, kneeAngleCounts);    // front right knee  
          }
