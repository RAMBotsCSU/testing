...
/*  if(RFB != 0){
  ...
  // most code below is straight from from opendogV3: 
    ...
    //walking
      ...
      Serial.println(previousStepMillis);
      Serial.println(currentMillis - previousStepMillis);
    ...
    if(gyro){
      ...
      Serial.print("Controls passed to kinematics!  ");
      Serial.print(fr_RFB);
      Serial.println(fr_RLR);
      kinematics (1, fr_RFB, fr_RLR, legLength1, 0, 0, 0, 1, (timerScale*0.8));   // front right
      // kinematics (1, 0, 0, 0, 0, 0, 0, 0, 0);   // front right
...
void danceMode(int dance1, int dance2, int dance3, int dance4){
  if(dance1){
    ...
    if(dance4Flag == 1){
      dance4Delay = 0;
      dance4Pos = dance4Pos - 5;
      if(dance4Pos <= minLegHeight){
        dance4Flag = 2;
      }
    }
